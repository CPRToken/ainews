...
export default function NavList({ data }: NavListProps) {
  ...
  const mainList = Array.isArray(data.children)
    ? data.children.filter((list) => list && list.subheader && list.subheader !== 'Common')
    : [];

  const commonList = Array.isArray(data.children)
    ? data.children.find((list) => list && list.subheader === 'Common')
    : null;
